...
{
	double critical_lateral_distance = carInfo.width / 2.0 + params.horizontalSafetyDistancel;//车侧向距离
	double critical_long_front_distance = carInfo.wheel_base / 2.0 + carInfo.length / 2.0 + params.verticalSafetyDistance;//车后轮之前的距离
	double critical_long_back_distance = carInfo.length / 2.0 + params.verticalSafetyDistance - carInfo.wheel_base / 2.0;//车后轮之后的距离
	int iCostIndex = 0;
...
{//计算各支路cost
	...
	for (unsigned int it = 0; it < laneRollOuts.size(); it++)
	{
		...
		tc.closest_obj_distance = params.horizonDistance;//障碍物距离即为预瞄距离
