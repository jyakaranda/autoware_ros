...
{//得到轨迹中从prev_index开始距离p最近的点
	...
	{
		d = distance2pointsSqr(trajectory[i].pos, p.pos);
		double angle_diff = UtilityH::AngleBetweenTwoAnglesPositive(trajectory[i].pos.a, p.pos.a) * RAD2DEG;//夹角角度
	...
	if (min_index < (int)trajectory.size() - 2)
	{
		...
		double norm2 = pointNorm(v_2);
		double dot_pro = v_1.x * v_2.x + v_1.y * v_2.y;
		double a = UtilityH::FixNegativeAngle(acos(dot_pro / (norm1 * norm2)));//余弦定理求角度
		if (a <= M_PI_2)
			min_index = min_index + 1;
	}
...
{//修正path中每个点的间隔
	...
	{
		d += hypot(path.at(ei).pos.x - path.at(ei - 1).pos.x, path.at(ei).pos.y - path.at(ei - 1).pos.y) + remaining;//ei总距离
		a = atan2(path.at(ei).pos.y - path.at(si).pos.y, path.at(ei).pos.x - path.at(si).pos.x);//ei偏离si的弧度
													 ...
													 const double &pathDensity, vector<WayPoint> &extractedPath, const double &SmoothDataWeight, const double &SmoothWeight, const double &SmoothTolerance)
{
	...
	{
		cout << endl
			 << "### Planner Z . Extracted Rollout Path is too Small, Size = " << extractedPath.size() << endl;
		return;
	}
												  ...
												  std::vector<WayPoint> &sampledPoints)
{
	...
	//calculate end index
	double start_distance = rollInSpeedFactor * speed + rollInMargin;//开始规划点距离
